...
const steps = ['Choose Payment Method', 'Waiting for Payment', 'Transaction Confirmation', 'Transaction Complete'];
              ...
              {blockchains.map((chain) => (
                <Card key={chain.name} className="overflow-hidden">
                  <div
                    ...
                    onClick={() => setExpandedChain(expandedChain === chain.name ? null : chain.name)}
                  >
                    <div className="flex items-center gap-4">
                      <div className="text-xl font-bold">{chain.name}</div>
                      <p className="text-muted-foreground">{chain.desc}</p>
                    </div>
                    <div className="text-sm text-muted-foreground">{expandedChain === chain.name ? '▲' : '▼'}</div>
                  </div>
                ...
                <CardContent className="p-8 text-center">
                  <div className="flex justify-center mb-6">
                    <div className="p-4 bg-white rounded-2xl shadow">
                      <QRCodeSVG
                        value={`${FindChainNamesByChainids(tx.blockchain.chain_id)}:${tx.blockchain.address}?amount=${tx.blockchain.crypto_amount}`}
                        size={260}
                        imageSettings={{
                  ...
                  <p className="text-muted-foreground">Send exactly this amount in one transaction</p>
                ...
                <CardContent className="p-6">
                  ...
                  {pasteTxId ? (
                    <div className="space-y-4">
                      <Input placeholder="Enter TxID" value={txid} onChange={(e) => setTxid(e.target.value)} />
                      <div className="flex gap-3">
                        <Button variant="outline" className="flex-1" onClick={() => setPasteTxId(false)}>
                          Cancel
                        </Button>
            ...
            <Card className="mt-10">
              <CardContent className="p-8 space-y-4 text-left">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{FindChainNamesByChainids(tx.blockchain.chain_id)}</span>
                </div>
                <div className="flex justify-between">
